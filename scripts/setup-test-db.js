...
async function setupTestDatabase() {
	...
	try {
		...
		if (result.rows.length > 0) {
			console.log(`La base de datos "${testDbName}" ya existe. Listo para tests.`);
		} else {
		...
		console.error('Error configurando la base de datos de test:', error.message);
		
		// Si el error es de conexión, dar más detalles
		if (error.code === 'ECONNREFUSED') {
			console.error('\nNo se pudo conectar a PostgreSQL.');
			console.error('Verifica que el contenedor de base de datos esté corriendo:');
